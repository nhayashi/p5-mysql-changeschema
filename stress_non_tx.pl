...
#            $dbh->commit;
        ...
        my $rand_id = select_rand_id($dbh);
        my $del_rows = delete_by_id( $dbh, $rand_id );
        if ( $del_rows != 1 ) {
            $act_key = 97;
#            print "ERROR delete\n";
#            $dbh->rollback;
...
#            $dbh->commit;
        ...
        my $rand_id = select_rand_id($dbh);
        my $upd_rows = update_rows( $dbh, $rand_id, "ccc" );
        if ( $upd_rows != 1 ) {
            $act_key = 96;
#            print "ERROR update\n";
#            $dbh->rollback;
...
sub select_rand_id {
    ...
    my $query = "select id from test2 where id>$rand order by id desc limit 1";
    return ( $dbh->selectrow_array($query) )[0];
}
